    ...
    unref(formRef).validate((valid: any) => {
        if (valid) {
            ...
            tableInfo.value.tableItems.unshift(
                ...dynamicValidateForm.value.formItemCfg.map((v: any) => {
                    return {
                        label: v.valueKey,
                        value: v.valueKey,
                    };
...
<template>
    <div class="app-main">
        ...
        <el-drawer v-model="show" title="设置" direction="ltr" size="70%">
            <template #default>
                <el-form ref="formRef" :model="dynamicValidateForm" label-width="auto" class="demo-dynamic">
                    <el-form-item
                        v-for="(item, index) in dynamicValidateForm.value.formItemCfg"
                        :key="item.key"
                        :label="'类目' + index"
                    ...
                    >
                        <div class="item-wrapper">
                            ...
                            <el-icon v-if="index === dynamicValidateForm.value.formItemCfg.length - 1" class="input-add" title="新增类目" @click.prevent="addFormIten"><CirclePlus /></el-icon>
        ...
        <el-table class="table-res" :data="tableInfo.showData" border style="width: 100%">
            <el-table-column :prop="it.value" :label="it.label" v-for="it in tableInfo.tableItems" :key="it.label" />
        </el-table>
    </div>
</template>
