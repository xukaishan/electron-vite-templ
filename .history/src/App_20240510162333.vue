...
const calc = (value: any) => {
    ...
    const res = dynamicValidateForm.value.formItemCfg.reduce((pre: any, cur: any) => {
        if (cur.valueKey !== 'other') {
            const includesVal = cur.value.split(',');
            const len = data.filter((it: any) => includesVal.includes(it)).length;
...
const onChange = (file: any) => {
    window.XlsxPopulate.fromDataAsync(file.raw).then((workbook: any) => {
        ...
        value.value = value;
        if (value.length) {
            options.value = value[0].map((v: any, i: number) => {
                return {
                    label: `${i}`,
                    value: `${i}`,
                };
...
<template>
    <div class="app-main">
        <div class="app-header">
            <el-select class="app-select" size="small" v-model="curColumnIndex" placeholder="选择统计的列" style="width: 140px">
